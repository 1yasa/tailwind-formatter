...
import { default_categories } from "../default";
...
export class FormatterConfigManager {
      ...
      private staticConfig: Omit<FormatterConfig, "prettierConfig" | "babelConfig"> | null = null;
       ...
       */
      public async getConfig(document: vscode.TextDocument): Promise<FormatterConfig> {
            try {
      ...
      private getStaticConfig(document: vscode.TextDocument): void {
            ...
            this.staticConfig = {
                  ...
                  multiLineClasses: config.get("lineFormatting.multiLineClasses") as boolean,
       ...
       */
      private async getPrettierConfig(document: vscode.TextDocument): Promise<Record<string, any>> {
            const extensionConfig = vscode.workspace.getConfiguration("tailwindFormatter");
            ...
            const extensionPrettierConfig = extensionConfig.get("prettier.config") as Record<
                  string,
                  any
            > | null;
            ...
            if (useProjectPrettierConfig) {
                  try {
                        ...
                        const projectPrettierConfig = await prettier.resolveConfig(documentPath);
